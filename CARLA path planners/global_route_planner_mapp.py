...
class GlobalRoutePlannerMAPP(object):
    """
    ...
    def trace_route(self, origin, destination, path_number):
        """
        This method returns list of (carla.Waypoint, RoadOption)
        from origin to destination
        """
        route_trace = []
        route = self._path_search(origin, destination, path_number)
        if route == "no paths found":
                return route
        else:
    ...
    def _path_search(self, origin, destination, path_number):
        """
        ...
        if paths :
            ...
            for path in routes:
                paths.append(path)
            # print('paths were assigned Successfully')
            paths.sort(key = len)


        if path_number > len(paths):
            route = "no paths found"
            # print('stop')

        else:

            route = paths[path_number]
            route.append(end[1])
            # print('appended new route',route)
        # print(route)
        return route
