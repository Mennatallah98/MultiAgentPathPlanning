...
# pub = rospy.Publisher('update_paths', String, queue_size=10)
class LocalPlanner(CompatibleNode):
    ...
    def __init__(self):
        ...
        global role_name
        global status  # variable carries the status of the vehicle 0--> parking  1--> retrieving
            ...
            if not self._waypoint_buffer:
                for i in range(self._buffer_size):
                    if self._waypoints_queue:
                        ...
                        print("reached goal")
                        # pub.publish(role_name)
                        if status == 1:   # retrieving a vehicle
                            # send signal when the vehicle reaches the gate to end its scripts
                            rospy.set_param('kill_{}'.format(role_name),1)
                            # rospy.set_param('kill_obs_{}'.format(role_name),1) # used when integrated with the rest of the project
                        
                        else: # parking a vehicle
                            rospy.set_param('kill_obs_{}'.format(role_name),1)
                            # start the parking action
                            command = "roslaunch mapp park.launch role_name:=" + role_name
