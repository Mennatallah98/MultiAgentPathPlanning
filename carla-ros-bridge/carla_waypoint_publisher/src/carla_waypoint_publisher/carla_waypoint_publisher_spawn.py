...
class CarlaToRosWaypointConverter(CompatibleNode):
    ...
    def __init__(self):
        ...
        gate_goal = '6.3,23,1.0,0.0,0,0,0' 
        self.pose_goal_msg.pose.position.x = float(eval(gate_goal.split(',')[0]))
        self.pose_goal_msg.pose.position.y = float(eval(gate_goal.split(',')[1]))
        self.pose_goal_msg.pose.position.z = float(eval(gate_goal.split(',')[2]))
        self.pose_goal_msg.pose.orientation.x = float(eval(gate_goal.split(',')[3]))
        self.pose_goal_msg.pose.orientation.y = float(eval(gate_goal.split(',')[4]))
        self.pose_goal_msg.pose.orientation.z = float(eval(gate_goal.split(',')[5]))
        self.pose_goal_msg.pose.orientation.w = float(eval(gate_goal.split(',')[6]))
        ...
        #     self.pose_goal_msg.pose.position.z = float(eval(self.goal_pose.split(',')[2]))
        #     self.pose_goal_msg.pose.orientation.x = float(eval(self.goal_pose.split(',')[3]))
        #     self.pose_goal_msg.pose.orientation.y = float(eval(self.goal_pose.split(',')[4]))
        #     self.pose_goal_msg.pose.orientation.z = float(eval(self.goal_pose.split(',')[5]))
        #     self.pose_goal_msg.pose.orientation.w = float(eval(self.goal_pose.split(',')[6]))
        #     self.goal = trans.ros_pose_to_carla_transform(self.pose_goal_msg.pose)
            # self.current_route = None
            # self.loginfo("Waiting for ego vehicle...")
            # self.on_tick = self.world.on_tick(self.find_ego_vehicle_actor)
        ...
        # set initial goal
        #self.goal = self.world.get_map().get_spawn_points()[30]
