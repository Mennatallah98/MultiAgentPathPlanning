...
sys.path.insert(0, '/home/mazz/carla/PythonAPI/carla')

import carla
from agents.navigation.global_route_planner_mapp import GlobalRoutePlannerMAPP
...
# pub = rospy.Publisher('new_agent', String, queue_size=10)
...
class CarlaToRosWaypointConverter(CompatibleNode):
    ...
    def __init__(self):
        ...
        self.pose_goal_msg = PoseStamped()

        super(CarlaToRosWaypointConverter, self).__init__('carla_waypoint_publisher_mapp')
        self.connect_to_carla()

        ############################ goal ########################
        self.goal_pose = self.get_param("goal_pose")
        #########################################################
        ...
        # pub.publish(self.role_name)
        self.waypoint_publisher = self.new_publisher(
            Path,
            '/carla/{}/waypoints_mapp'.format(self.role_name),
            QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL))

        # initialize ros services
        self.get_waypoint_service = self.new_service(
            GetWaypoint,
            '/carla_waypoint_publisher_mapp/{}/get_waypoint'.format(self.role_name),
            self.get_waypoint)
        self.get_actor_waypoint_service = self.new_service(
            GetActorWaypoint,
            '/carla_waypoint_publisher_mapp/{}/get_actor_waypoint'.format(self.role_name),
            self.get_actor_waypoint)


        self.pose_goal_msg.pose.position.x = float(eval(self.goal_pose.split(',')[0]))
        self.pose_goal_msg.pose.position.y = float(eval(self.goal_pose.split(',')[1]))
        self.pose_goal_msg.pose.position.z = 1.0
        self.pose_goal_msg.pose.orientation.x = 0.0
        self.pose_goal_msg.pose.orientation.y = 0.0
        self.pose_goal_msg.pose.orientation.z = 0.0
        self.pose_goal_msg.pose.orientation.w = 0.0
        self.goal = trans.ros_pose_to_carla_transform(self.pose_goal_msg.pose)
        
        self.current_route = None
        rospy.set_param("given",1)
    ...
    def reroute(self):
        ...
        if self.ego_vehicle is None or self.goal is None:
            ...
            self.current_route = self.calculate_route(self.goal, path_number)
            # print(path_number)
    ...
    def calculate_route(self, goal, path_number):
        """
        ...
        grp_mapp = GlobalRoutePlannerMAPP(self.world.get_map(), sampling_resolution=1)
        route = grp_mapp.trace_route(self.ego_vehicle.get_location(),
                                carla.Location(goal.location.x,
                                               goal.location.y,
                                               goal.location.z), path_number)
    ...
    def publish_waypoints(self):
        ...
        if self.current_route == "no paths found":
            msg.header.frame_id= "no paths found"
...
def main(args=None):
    ...
    main function
    """
    roscomp.init('carla_waypoint_publisher_mapp', args)

    global waypoint_converter
