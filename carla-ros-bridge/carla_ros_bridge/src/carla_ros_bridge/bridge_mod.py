...
def onTickCallback():
    print("entered the custom cb#################")
    global allowedToTick
    allowedToTick = 1
...
class CarlaRosBridge(CompatibleNode):
    ...
    def initialize_bridge(self, carla_world, params):
        ...
        if self.sync_mode:
            ...
            self.carla_world.on_tick(lambda world_snapshot: onTickCallback())
    ...
    def _synchronous_mode_update(self):
        ...
        while not self.shutdown.is_set() and roscomp.ok():
            ...
            #################################
            #self.carla_world.tick() #original
            #################################
            while(not allowedToTick):
                waitVariable = 0
            ...
            # tick_url = 'http://10.10.30.229:4000/tick'
            # res2 = requests.post(tick_url, json={})
            allowedToTick = 0

 

            frame = self.carla_world.get_snapshot().frame
            ##################################
...
def main(args=None):
    ...
    connect_url = 'http://10.10.30.229:4000/connect'
    res1 = requests.post(connect_url, json={})
    print('connect+++++++++++++++++++++')
    ...
    try:
        ...
        # check carla versioncarla_world
        ...
        dis_url = 'http://10.10.30.229:4000/disconnect'
        res3 = requests.post(dis_url, json={})
        roscomp.shutdown()
